...
def crop_first(image, polygons, scale =10):
    ...
    for polygon in polygons_new:
        # print(polygon['poly'])
        rect = cv2.minAreaRect(polygon['poly'].astype(np.int32))
        box = cv2.boxPoints(rect)
        box = np.int0(box)
        text_polys.append([box[0], box[1], box[2], box[3]])
        if '#' not in polygon['text']:
...
class Compose(DataProcess):
    ...
    def process(self, img, pts=None):
        for t in self.transforms:
            img, pts = t(img, pts)
        return img, pts


class Normalize(DataProcess):
    def __init__(self, **kwargs):
        self.mean = np.array(kwargs.get('means'))
        self.std = np.array(kwargs.get('stds'))

    def process(self, image, polygons=None):
        # image = data['image']
        image = image.astype(np.float32)
        # print(image.dtype, self.mean)
        image /= 255.0
        image -= self.mean
        image /= self.std
...
class RandomMirror(DataProcess):
    # 镜像
    def __init__(self, **kwargs):
        pass
    ...
    def process(self, image, polygons=None):
        ...
        if random.random()< 0.3:
            ...
            for polygon in polygons:
                polygon['poly'][:, 0] = width - polygon['poly'][:, 0]
        return image, polygons


class AugmentColor(DataProcess):
    # 颜色增强（添加噪声）
    def __init__(self):
    ...
    def process(self, img, polygons=None):
...
class RandomContrast(DataProcess):
...
class RandomBrightness(DataProcess):
...
class RandomErasing(DataProcess):
...
class RandomMixUp(DataProcess):
    def __init__(self, mixup_alpha=2):
        self.mixup_alpha = mixup_alpha

    def process(self, img1, img2, label1=[], label2=[]):
...
class Rotate(DataProcess):
    ...
    def process(self, img, polygons=None):
        ...
        if polygons is not None:
            for polygon in polygons:
                x, y = self.rotate(center, polygon['poly'], angle)
                pts = np.vstack([x, y]).T
                polygon['poly'] = pts
        return img, polygons


class RotatePadding(DataProcess):
    def __init__(self, **kwargs):
    ...
    def process(self, image, polygons=None, scale=1.0):
        ...
        if polygons is not None:
            for polygon in polygons:
                x, y = self.rotate(center, polygon['poly'], angle,movSize,scale)
                pts = np.vstack([x, y]).T
                polygon['poly'] = pts
        return image, polygons


class SquarePadding(DataProcess):
...
class RandomImgCropPatch(DataProcess):
    ...
    def CropWH(self, image, cut_w, cut_h, polygons=None):
        ...
        for idx in selected_polys:
            ...
            polygon['poly'] -= np.array([xmin, ymin])
    ...
    def process(self, images, polygons_list=None):
        I_x, I_y = 1024,1024
        ...
        for polygon in new_polygons[1]:
            polygon['poly'] += np.array([w, 0])
        for polygon in new_polygons[2]:
            polygon['poly'] += np.array([0, h])
        for polygon in new_polygons[3]:
            polygon['poly'] += np.array([w, h])
...
class RandomCropFlip(DataProcess):

    def __init__(self, **kwargs):
        self.scale = 10
        self.ratio = 0.2
        self.epsilon = 10.0
        self.min_crop_side_ratio = 0.01
    ...
    def process(self, image, polygons=None):
        ...
        while attempt < 10:
            ...
            for polygon in polygons:
                ...
                ppi = Polygon(polygon['poly']).buffer(0)
        ...
        if len(polygons_new) == 0:
            ...
            if select_type == 0:
                img = np.ascontiguousarray(cropped[:, ::-1])
                for polygon in polygons_new:
                    polygon['poly'][:, 0] = width - polygon['poly'][:, 0] + 2 * xmin
            elif select_type == 1:
                img = np.ascontiguousarray(cropped[::-1, :])
                for polygon in polygons_new:
                    polygon['poly'][:, 1] = height - polygon['poly'][:, 1] + 2 * ymin
            else:
                img = np.ascontiguousarray(cropped[::-1, ::-1])
                for polygon in polygons_new:
                    polygon['poly'][:, 0] = width - polygon['poly'][:, 0] + 2 * xmin
...
class RandomResizedCrop(DataProcess):
    def __init__(self, **kwargs):
        self.scale = 10
        self.epsilon = 1e-2
        self.min_crop_side_ratio = 0.1

    def process(self, image, polygons):
...
class RandomResizeScale(DataProcess):
    def __init__(self, **kwargs):
    ...
    def process(self, image, polygons=None):
        ...
        if polygons is not None:
            for polygon in polygons:
                polygon['poly'] = polygon['poly'] * scales

        return image, polygons


class Resize(DataProcess):
    def __init__(self, **kwargs):
        self.size = kwargs.get('size')
...
class ResizeSquare(DataProcess):
    def __init__(self, **kwargs):
        self.size = kwargs.get('size')
...
class ResizeLimitSquare(DataProcess):
    def __init__(self, **kwargs):
        self.size = kwargs.get('size')
        self.ratio = kwargs.get('ratio')
        self.SP = SquarePadding()
...
class RandomResizePadding(DataProcess):
...
class RandomDistortion(DataProcess):
    def __init__(self, **kwargs):
        transform_dict = kwargs.get('transform_dict')
        # print(transform_dict)
        self.transforms = [(transform_type_dict[k], transform_dict[k]) for k in transform_dict]
        self.prob = 0.5

    def process(self, img, target):
        if random.random() > self.prob:
            return img, target
        # print(img.shape, img.dtype, img.max())
        out = Image.fromarray(np.round(img).astype(np.uint8))
...
class Augmentation(DataProcess):
...
class BaseTransform(DataProcess):
    def __init__(self, size, mean, std):
        self.size = size
        self.mean = mean
        self.std = std
...
class BaseTransformNresize(DataProcess):
